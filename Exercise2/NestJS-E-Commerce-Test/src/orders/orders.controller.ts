import { Controller, Injectable, Req, Post, Res, Get } from '@nestjs/common';
    ...
    async getProducts(@Req() req: Request, @Res() res: Response) {
        try {
            ...
            if (error instanceof Error) {
                res.status(400).json({ message: `Failed to get products: ${error.message}` });
            } else {
                res.status(500).json({ message: "An unexpected error occurred while getting products." });
            }
        }
    }
